"""
Evolutionary Agent: gen3_agent_006
==================================

Metadata:
{
  "generation": 3,
  "fitness": 109.39599999999498,
  "fighting_style": "evolved",
  "win_rate": 0.0
}

Code Hash: f32ca02b0c422243
Serialization Version: 1.0
"""

# Agent Code:
import numpy as np
import random
import math

def get_action(state):
    # Extract and validate core strategic information
    distance = max(0.0, min(1.0, state[22]))
    relative_pos = max(-1.0, min(1.0, state[23]))
    health_advantage = max(-1.0, min(1.0, state[25]))
    height_diff = state[24] if len(state) > 24 else 0.0
    
    # Extract detailed fighter status with bounds checking
    my_health = max(0.0, min(1.0, state[2]))
    my_pos_x = max(0.0, min(1.0, state[0]))
    my_velocity_x = max(-1.0, min(1.0, state[3]))
    my_attack_status = max(0.0, min(1.0, state[7]))
    my_block_status = max(0.0, min(1.0, state[8]))
    my_projectile_cooldown = max(0.0, state[9])
    
    opponent_health = max(0.0, min(1.0, state[13]))
    opponent_pos_x = max(0.0, min(1.0, state[11]))
    opponent_velocity_x = max(-1.0, min(1.0, state[14]))
    opponent_attack_status = max(0.0, min(1.0, state[18]))
    opponent_block_status = max(0.0, min(1.0, state[19]))
    opponent_projectile_cooldown = max(0.0, state[20])
    
    # Enhanced tactical range definitions
    ultra_close = 0.06
    close_range = 0.14
    medium_range = 0.28
    far_range = 0.45
    max_range = 0.65
    
    # Dynamic threshold calculations
    critical_health = 0.18
    low_health = 0.35
    winning_threshold = 0.25
    dominating_threshold = 0.50
    
    # Cooldown and readiness analysis
    projectile_ready = my_projectile_cooldown < 0.20
    projectile_charging = my_projectile_cooldown < 0.40
    opponent_projectile_ready = opponent_projectile_cooldown < 0.25
    opponent_projectile_threat = opponent_projectile_cooldown < 0.45
    
    # Advanced positioning analysis
    stage_center = 0.5
    my_corner_distance = min(my_pos_x, 1.0 - my_pos_x)
    opponent_corner_distance = min(opponent_pos_x, 1.0 - opponent_pos_x)
    i_am_cornered = my_corner_distance < 0.12
    i_am_near_corner = my_corner_distance < 0.25
    opponent_cornered = opponent_corner_distance < 0.12
    opponent_near_corner = opponent_corner_distance < 0.25
    
    # Enhanced behavioral pattern recognition
    opponent_aggressive = opponent_attack_status > 0.4
    opponent_very_aggressive = opponent_attack_status > 0.7
    opponent_defensive = opponent_block_status > 0.5
    opponent_very_defensive = opponent_block_status > 0.8
    
    # Movement pattern analysis
    opponent_advancing = (relative_pos > 0 and opponent_velocity_x > 0.05) or (relative_pos < 0 and opponent_velocity_x < -0.05)
    opponent_retreating = (relative_pos > 0 and opponent_velocity_x < -0.05) or (relative_pos < 0 and opponent_velocity_x > 0.05)
    opponent_stationary = abs(opponent_velocity_x) < 0.05
    
    # Combat momentum and flow analysis
    total_momentum = abs(my_velocity_x) + abs(opponent_velocity_x)
    combat_intensity = my_attack_status + opponent_attack_status
    health_ratio = my_health / max(opponent_health, 0.05)
    pressure_level = combat_intensity + (1.0 - distance)
    
    # Strategic state determination
    crisis_mode = my_health < critical_health or health_advantage < -0.7
    survival_mode = my_health < low_health or health_advantage < -0.4
    balanced_mode = abs(health_advantage) < 0.3 and my_health > 0.3
    advantage_mode = health_advantage > 0.2 and my_health > 0.4
    finishing_mode = opponent_health < 0.20 and health_advantage > 0.1
    domination_mode = health_advantage > dominating_threshold
    
    # Randomization for unpredictability
    chaos_factor = random.random()
    tactical_roll = random.random()
    timing_variance = random.random()
    
    # CRISIS MODE - Desperate survival tactics
    if crisis_mode:
        if distance < ultra_close:
            if opponent_very_aggressive:
                return 6  # Emergency block
            elif i_am_cornered and chaos_factor < 0.4:
                return 3  # Desperate jump escape
            else:
                # Create immediate space with blocking movement
                escape_direction = 8 if my_pos_x < stage_center else 7
                return escape_direction
        
        elif distance < close_range:
            if opponent_advancing and opponent_aggressive:
                return 6  # Block the approach
            elif i_am_cornered:
                # Corner escape priority
                if projectile_ready and chaos_factor < 0.3:
                    return 9  # Projectile to create space
                else:
                    escape_direction = 8 if my_pos_x < stage_center else 7
                    return escape_direction
            else:
                # Controlled retreat
                if projectile_ready and tactical_roll < 0.5:
                    return 9  # Zone while retreating
                else:
                    retreat_direction = 2 if relative_pos < 0 else 1
                    return retreat_direction
        
        elif distance < medium_range:
            if opponent_projectile_threat:
                # Evasive maneuvers
                if chaos_factor < 0.4:
                    return 3  # Jump over projectile
                else:
                    dodge_direction = 2 if chaos_factor < 0.7 else 1
                    return dodge_direction
            elif projectile_ready:
                return 9  # Counter-zone
            else:
                # Maintain safe distance
                if opponent_advancing:
                    retreat_direction = 2 if relative_pos < 0 else 1
                    return retreat_direction
                else:
                    return 0  # Wait and recover
        
        else:
            # Safe distance - recover and plan
            if projectile_ready:
                return 9  # Long range harassment
            else:
                return 0  # Wait for cooldown
    
    # FINISHING MODE - Close out the match aggressively
    elif finishing_mode:
        if opponent_cornered:
            if distance < ultra_close:
                if opponent_very_defensive:
                    # Break through defense
                    if chaos_factor < 0.25:
                        return 9 if projectile_ready else 3  # Throw or jump mix
                    elif chaos_factor < 0.6:
                        return 5  # Heavy kick
                    else:
                        return 4  # Fast punch
                else:
                    # Go for the kill
                    return 5 if tactical_roll < 0.7 else 4
            
            elif distance < close_range:
                if opponent_defensive:
                    # Pressure with mix-ups
                    mix_option = chaos_factor
                    if mix_option < 0.3:
                        return 9 if projectile_ready else 5
                    elif mix_option < 0.6:
                        return 3  # Jump attack